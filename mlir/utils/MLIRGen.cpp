...
SgInitializedName *get_sage_symbol(SgExpression *node) {
  ...
  std::cout << "======  Checking a SgExpression symbol: "
...
mlir::Value convert_binary_op(mlir::OpBuilder &builder, SgExpression *node) {
  ...
  case V_SgAddOp: {
    ...
    result = builder.create<mlir::LLVM::FAddOp>(location, left_operand,
                                                right_operand, fmf);
    break;
  }
  case V_SgMultiplyOp: {
    ...
    mlir::Value left_operand = convert_op(builder, op->get_lhs_operand());
    mlir::Value right_operand = convert_op(builder, op->get_rhs_operand());
    result = builder.create<mlir::LLVM::FMulOp>(location, left_operand,
                                                right_operand, fmf);
    break;
  }
  case V_SgDivideOp: {
    ...
    assert(op != NULL);
    mlir::Value left_operand =
        convert_op(builder, isSgBinaryOp(node)->get_lhs_operand());
    mlir::Value right_operand =
        convert_op(builder, isSgBinaryOp(node)->get_rhs_operand());
    result = builder.create<mlir::LLVM::FDivOp>(location, left_operand,
                                                right_operand, fmf);
    break;
  }
  case V_SgAssignOp: {
...
mlir::Value convert_op(mlir::OpBuilder &builder, SgExpression *node) {
  ...
  cuda_kernel->get_exec_config();
          ...
          SgExpression* num_threads_per_block_expression =
  kernel_config->get_blocks(); num_threads_per_block = convert_op(builder,
  num_threads_per_block_expression);

          SgExpressionPtrList& func_parameters =
  cuda_kernel->get_args()->get_expressions(); std::map<SgVariableSymbol *,
  ParallelData *> parallel_data = analyze_cuda_parallel_data(cuda_kernel);
          std::map<SgVariableSymbol *, ParallelData *>::iterator data_iter;
          std::vector<mlir::Value> value_list;
          int i = 0;
          for (data_iter = parallel_data.begin(); data_iter !=
  parallel_data.end(); data_iter++) { ParallelData * iter = data_iter->second;
            ...
            llvm::ArrayRef<llvm::StringRef> parallel_data_string = {symbol,
  sharing_property, sharing_visibility, mapping_property, mapping_visibility,
  data_access}; mlir::ArrayAttr parallel_data =
  builder.getStrArrayAttr(parallel_data_string); SgInitializedName *sage_symbol
  = get_sage_symbol(func_parameters.at(i)); i++; mlir::Value symbol_value =
  nullptr; if (sage_symbol != NULL) { symbol_value =
  upir::symbol_table[sage_symbol].first;
            }
            mlir::Value parallel_data_value =
  builder.create<mlir::upir::ParallelDataInfoOp>(location,
  builder.getNoneType(), parallel_data, symbol_value);
            value_list.push_back(parallel_data_value);

          }
          llvm::ArrayRef<mlir::Value> parallel_data_values =
  llvm::ArrayRef<mlir::Value>(value_list); mlir::ValueRange parallel_data_range
  = mlir::ValueRange(parallel_data_values);

          mlir::StringAttr device =
  builder.getStringAttr(llvm::StringRef("nvptx")); mlir::upir::TaskOp
  task_target = builder.create<mlir::upir::TaskOp>(location, nullptr, device,
  nullptr, mlir::ValueRange(), parallel_data_range, nullptr); mlir::Region
  &task_body = task_target.getRegion(); builder.createBlock(&task_body);

          mlir::upir::SpmdOp spmd_grid =
  builder.create<mlir::upir::SpmdOp>(location, num_blocks, nullptr,
  mlir::ValueRange(), parallel_data_range, nullptr); mlir::Region
  &spmd_grid_body = spmd_grid.getRegion(); builder.createBlock(&spmd_grid_body);

          mlir::upir::SpmdOp spmd_block =
  builder.create<mlir::upir::SpmdOp>(location, num_threads_per_block, nullptr,
  mlir::ValueRange(), parallel_data_range, nullptr); mlir::Region
  &spmd_block_body = spmd_block.getRegion();
  ...
  global_scope->lookup_function_symbol(func_name_string); assert(func_symbol !=
  NULL);

          std::vector<mlir::Value> args;
          SgExpressionPtrList::const_iterator iter;
          for (iter = func_parameters.begin(); iter != func_parameters.end();
  iter++) { mlir::Value arg = nullptr; SgInitializedName* symbol =
  get_sage_symbol(*iter); assert(symbol != NULL); arg =
  upir::symbol_table.at(symbol).first; args.push_back(arg);
          }
          llvm::ArrayRef<mlir::Value> args_value =
  llvm::ArrayRef<mlir::Value>(args); mlir::ValueRange func_parameter_values =
  mlir::ValueRange(args_value); builder.create<mlir::CallOp>(location,
  llvm::StringRef(func_name_string), mlir::TypeRange(), func_parameter_values);
...
*/
  ...
  case V_SgIntVal: {
    result = builder.create<mlir::arith::ConstantIntOp>(
        location, std::stoi(node->unparseToString()), 32);
    break;
  }
  default: {
    printf("Unknown SgExpression!\n");
    std::cout << "======  Meet a new SgExpression Op: "
              << node->sage_class_name() << " ======\n";
  }
  }
  return result;
...
void convert_statement(mlir::OpBuilder &builder, SgStatement *node) {
  ...
  case V_SgUpirSpmdStatement: {
    ...
    // analyze_parallel_data(isSgSourceFile(&(upir::g_project->get_file(0))));
    // std::map<SgVariableSymbol *, ParallelData *>::iterator data_iter;
    ...
    for (size_t i = 0; i < data_fields.size(); i++) {
      ...
      for (std::list<SgUpirDataItemField *>::iterator iter = data_items.begin();
        ...
        llvm::ArrayRef<llvm::StringRef> parallel_data_string = {
            symbol,           sharing_property,   sharing_visibility,
            mapping_property, mapping_visibility, data_access};
        mlir::ArrayAttr parallel_data =
            builder.getStrArrayAttr(parallel_data_string);
        mlir::Value parallel_data_value =
            builder.create<mlir::upir::ParallelDataInfoOp>(
                location, builder.getNoneType(), parallel_data, nullptr);
        value_list.push_back(parallel_data_value);
      }
    }
    llvm::ArrayRef<mlir::Value> parallel_data_values =
        llvm::ArrayRef<mlir::Value>(value_list);
    mlir::ValueRange parallel_data_range =
        mlir::ValueRange(parallel_data_values);
    ...
    if (OmpSupport::hasClause(target, V_SgUpirNumUnitsField)) {
      Rose_STL_Container<SgOmpClause *> num_threads_clauses =
          OmpSupport::getClause(target, V_SgUpirNumUnitsField);
      SgUpirNumUnitsField *num_threads_clause =
          isSgUpirNumUnitsField(num_threads_clauses[0]);
      std::string num_threads_string =
          num_threads_clause->get_expression()->unparseToString();
      int32_t num_thread_value = std::stoi(num_threads_string);
      num_threads = builder.create<mlir::arith::ConstantIntOp>(
          location, num_thread_value, 32);
    }
    mlir::upir::SpmdOp spmd = builder.create<mlir::upir::SpmdOp>(
        location, num_threads, nullptr, mlir::ValueRange(), parallel_data_range,
        nullptr);
  ...
  case V_SgOmpDoStatement: {
    ...
    if (OmpSupport::hasClause(omp_target, V_SgOmpCollapseClause)) {
      Rose_STL_Container<SgOmpClause *> collapse_clauses =
          OmpSupport::getClause(omp_target, V_SgOmpCollapseClause);
      SgOmpCollapseClause *collapse_clause =
          isSgOmpCollapseClause(collapse_clauses[0]);
      std::string collapse_string =
          collapse_clause->get_expression()->unparseToString();
      int32_t collapse_value = std::stoi(collapse_string);
      collapse = builder.create<mlir::arith::ConstantIntOp>(location,
                                                            collapse_value, 32);
    }
    ...
    if (for_loop != NULL) {
      is_canonical = SageInterface::isCanonicalForLoop(
          for_loop, &for_index, &for_lower, &for_upper, &for_stride, NULL,
          &isIncremental);
    } else {
      SageInterface::doLoopNormalization(do_loop);
      is_canonical = SageInterface::isCanonicalDoLoop(
          do_loop, &for_index, &for_lower, &for_upper, &for_stride, NULL,
          &isIncremental, NULL);
    }
    ...
    if (for_lower_symbol != NULL) {
      ...
      if (upir::symbol_table.count(for_lower) != 0) {
        ...
        lower_bound = builder.create<mlir::arith::ConstantIndexOp>(
            location, std::stoi(for_lower->unparseToString()));
        upir::symbol_table[for_lower] =
            std::make_pair(lower_bound, for_lower_symbol);
      }
    }
    ...
    if (for_upper_symbol != NULL) {
      ...
      upper_bound = builder.create<mlir::arith::ConstantIndexOp>(
          location, std::stoi(for_upper->unparseToString()));
    }
    ...
    if (for_stride_symbol != NULL) {
      ...
      if (upir::symbol_table.count(for_stride_symbol) != 0) {
        ...
        stride = builder.create<mlir::arith::ConstantIndexOp>(
            location, std::stoi(for_stride->unparseToString()));
        upir::symbol_table[for_stride] =
            std::make_pair(stride, for_stride_symbol);
      }
    }

    mlir::upir::WorkshareOp workshare_target =
        builder.create<mlir::upir::WorkshareOp>(
            location, nullptr, lower_bound, upper_bound, stride, nullptr,
            nullptr, collapse, mlir::ValueRange(), nullptr, mlir::ValueRange(),
            nullptr);
  ...
  case V_SgUpirTaskStatement: {
    ...
    mlir::upir::TaskOp task_target = builder.create<mlir::upir::TaskOp>(
        location, nullptr, device, nullptr, mlir::ValueRange(),
  ...
  case V_SgOmpTaskStatement: {
    ...
    mlir::upir::TaskOp task = builder.create<mlir::upir::TaskOp>(
        location, nullptr, nullptr, nullptr, mlir::ValueRange(),
        mlir::ValueRange(), nullptr);
  ...
  case V_SgFortranDo: {
    ...
    mlir::scf::ForOp loop = builder.create<mlir::scf::ForOp>(
        location, lower_bound, upper_bound, stride, loop_value);
...
public:
  UpirSageAST(mlir::ModuleOp &__root, mlir::OpBuilder &__builder)
      : root(__root), builder(__builder){};
...
protected:
  ...
  evaluateInheritedAttribute(SgNode *, InheritedAttribute) override;
};

InheritedAttribute
UpirSageAST::evaluateInheritedAttribute(SgNode *node,
                                        InheritedAttribute attribute) {
  if (isSgStatement(node)) {
    std::cout << "SgNode: " << node->sage_class_name()
              << " at line: " << node->get_startOfConstruct()->get_line()
              << "\n";
  }
  switch (node->variantT()) {
  case V_SgFunctionDefinition: {
    ...
    SgInitializedNamePtrList function_parameters =
        target->get_declaration()->get_args();
    SgInitializedNamePtrList::const_iterator iter;
    for (iter = function_parameters.begin(); iter != function_parameters.end();
         iter++) {
      mlir::Type type = nullptr;
      SgInitializedName *symbol = *iter;
      SgType *symbol_type = symbol->get_type();
    ...
    llvm::StringRef func_name =
        std::string(target->get_declaration()->get_name().getString());

    std::cout << "Create a base function...." << std::endl;
    mlir::func::FuncOp func =
        mlir::func::FuncOp::create(location, func_name, func_type);
    ...
    for (iter = function_parameters.begin(); iter != function_parameters.end();
         iter++) {
      SgInitializedName *symbol = *iter;
      upir::symbol_table[symbol] =
          std::make_pair(entryBlock.getArgument(i), symbol);
      i++;
    }
  ...
  default: {
    ...
    return InheritedAttribute(attribute.frontier, true, attribute.depth + 1,
                              node);
  }
  };
};
